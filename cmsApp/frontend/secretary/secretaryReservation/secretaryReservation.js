document.addEventListener('DOMContentLoaded', () => {
    ...
    function openEditModal(reservationId) {
        ...
        document.getElementById('editStatus').value = rec.status || 'Pending';
    ...
    function openDocModal(reservationId) {
        ...
        if (rec.clientValidId) {
            ...
            // Use the getDocument.php API instead of direct file URL
            const docUrl = `/stJohnCmsApp/cms.api/getDocument.php?doc=client-id&id=${reservationId}`;
                    ...
                    .then(response => {
                        if (response.ok) {
                            ...
                            imgPreview.onload = function() {
                                console.log('Client ID image loaded successfully');
                            };
                            
                            imgPreview.onerror = function() {
                                console.error('Failed to load image:', docUrl);
                                imgPreview.classList.add('d-none');
                                docFilename.textContent = `Failed to load image: ${rec.clientValidId}. Click Download to view.`;
                            };
                        } else {
                            // File doesn't exist, show error message and placeholder
                            imgPreview.classList.add('d-none');
                            noImagePlaceholder.classList.remove('d-none');
                            noImagePlaceholder.innerHTML = `
                                <i class="fas fa-exclamation-triangle fa-3x mb-3 text-warning"></i>
                                <p class="mb-0">File not found: ${rec.clientValidId}</p>
                                <small class="text-muted">The file may have been deleted or never uploaded.</small>
                            `;
                            docFilename.textContent = `File not found: ${rec.clientValidId}`;
                        }
                    })
                    .catch(error => {
                        ...
                        noImagePlaceholder.innerHTML = `
                            ...
                            <p class="mb-0">Error loading file: ${rec.clientValidId}</p>
                            <small class="text-muted">Please try again or contact support.</small>
                        `;
                        docFilename.textContent = `Error loading file: ${rec.clientValidId}`;
                    });
                
            } else if (fileExt === 'pdf') {
                docFilename.textContent += ' (PDF Document - Click Download to view)';
            } else {
                docFilename.textContent += ' (Document available - Click Download to view)';
            }
        } else {
            ...
            noImagePlaceholder.innerHTML = `
                ...
                <p class="mb-0">No Client ID uploaded</p>
                <small class="text-muted">Click "Add" to upload a Client ID document.</small>
            `;
        }
        docModal.show();
    }
